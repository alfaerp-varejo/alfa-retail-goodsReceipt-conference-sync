...
export class BtpGoodsReceiptConferenceService {
    ...
    async getList(): Promise<GoodsReceiptConference[]> {
        try {
            ...
            const res = await axios.get(`${this.env.host}/odata/v4/goods-receipt-conference-service-approver/GoodsReceiptConference?$top=100&$expand=_itens&$filter=lastSyncStatus_code eq 'P'`, { headers });
    ...
    async setSyncFields(ID: UUID, sync: Sync): Promise<boolean> {
        try {
            ...
            const res = await axios.patch(`${this.env.host}/odata/v4/goods-receipt-conference-service-approve/GoodsReceiptConferences(ID=${ID})`, sync, { headers });
            const response = res.data.value ?? res.data;
